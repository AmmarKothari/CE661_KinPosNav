...
% Uses magnetometer/accelerometer to estimate heading along with gyros
...
ImgFolder = 'KF_GPS_IMU_MAG_SensorLoop';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
H_IMU = KF_GPS_IMU_MAG.IMUstateMeasurementTransition();
H_GPS = KF_GPS_IMU_MAG.GPSstateMeasurementTransition();

% R =  noise covariance
IMU_noise = 100; GYRO_noise = 1; GPS_noise = 10; MAGACC_noise = 100;
...
R_IMU = diag([IMU_noise; IMU_noise; IMU_noise;GYRO_noise; GYRO_noise; GYRO_noise; MAGACC_noise; MAGACC_noise; MAGACC_noise]);

KF = KF_GPS_IMU_MAG(Q, H_GPS, H_IMU, R_GPS, R_IMU);
IMU = KF_GPS_IMU_MAG(Q, H_GPS, H_IMU, R_GPS, R_IMU);
GPS = KF_GPS_IMU_MAG(Q, H_GPS, H_IMU, R_GPS, R_IMU);
GPS_t = KF_GPS_IMU_MAG(Q, H_GPS, H_IMU, R_GPS, R_IMU);
...
% 
% % KF over GPS, IMU, and Mag data
figure('Name', 'KF over GPS, IMU, and Mag', 'units','normalized','outerposition',[0 0 1 1])
subplot(3,1,1)
plot(KF.t_all(1:pts:end)-KF.t_all(1), KF.x_all(1:pts:end), 'rx-', t_step_GPS, data.GPS_data(1:end-1,1), 'g^')
xlabel('Time(s)'); ylabel('North(m)');
legend('KF', 'GPS')
title('Kalman Filter over GPS, IMU, and Mag Readings');
% East
subplot(3,1,2)
...
xlabel('Time(s)'); ylabel('Down(m)');
ylim([-20,500]);
saveas(gcf, fullfile(ImgFolder, 'KFOverGPS.png'));
